...
@REM set "FS_INCLUDE_SOURCE=%SCRIPT_DIR%\fs_include"
@REM set "FS_INCLUDE_DEST=%BUILD_DIR%\fs_include"

@REM if exist "%FS_INCLUDE_SOURCE%" (
@REM     echo %GREEN%Kopiere fs_include in das Build-Verzeichnis...%RESET%
@REM     xcopy /E /I /Y "%FS_INCLUDE_SOURCE%" "%FS_INCLUDE_DEST%\" >nul
@REM ) else (
@REM     echo %RED%[FEHLER] fs_include nicht gefunden in: %FS_INCLUDE_SOURCE%%RESET%
@REM     exit /b 1
@REM )

@REM :: 2. Pfade für CMake setzen
@REM set "ASSIMP_ROOT=%FS_INCLUDE_DEST%\assimp-windows64-5.2.5-r3"
@REM set "OPENAL_ROOT=%FS_INCLUDE_DEST%\openal-1.24.2-r1-windows64-13245988487"
...
if exist "%BUILD_DIR%\phoenix-firestorm\build-vc170-64\newview\Release" (
    ...
    xcopy /Y "%SCRIPT_DIR%fs_include\alut.dll" "%BUILD_DIR%\phoenix-firestorm\build-vc170-64\newview\Release\"
    xcopy /Y "%SCRIPT_DIR%fs_include\OpenAL32.dll" "%BUILD_DIR%\phoenix-firestorm\build-vc170-64\newview\Release\"
    autobuild package -A 64 --config-file "%AUTO_BUILD_CONFIG%"
) 

:: Starte kopieren mit 3_firestorm_Release.bat
call "%SCRIPT_DIR%3_firestorm_Release.bat"
